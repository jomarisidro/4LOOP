...
export default function EncodingForm() {
  ...
  const handleVerify = async (_id) => {
    const encoding = requests.find((req) => req._id === _id);
    console.log('✅ Encoding the request:', encoding);

    try {


      localStorage.setItem('encodingRequestId', _id);
      router.push(`/officers/workbench/encodingonlinerequest?id=${_id}`);
    } catch (err) {
    ...
    queryClient.invalidateQueries(['encoding-requests']);
  };
